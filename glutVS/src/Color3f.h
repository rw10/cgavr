...
{
public:
	Color3f(float red, float green, float blue, float alpha = 1.0f) :
		r(red), g(green), b(blue), a(alpha){}
	~Color3f(){}

	float r, g, b, a;

	Color3f operator*(const double& multiplier) const
	{
		return Color3f(
			(float)fmin(1.0f, r * multiplier),
			(float)fmin(1.0f, g * multiplier),
			(float)fmin(1.0f, b * multiplier),
			(float)fmin(1.0f, a * multiplier)
		);
	}
...
#define Red = Color3f (1, 0, 0)
#define Green = Color3f(0, 1, 0)
#define Blue = Color3f(0, 0, 1)
#define Yellow = Color3f(1, 1, 0)
#define Cyan = Color3f(0, 1, 1)
#define Magenta = Color3f(1, 0, 1)
#define Transparent = Color3f(1,1,1,0)
#define White = Color3f(1,1,1)
#define Black = Color3f(0,0,0)
